...
namespace {
class MatMulRVVPass
    : public PassWrapper<MatMulRVVPass, OperationPass<ModuleOp>> {

public:
  MLIR_DEFINE_EXPLICIT_INTERNAL_INLINE_TYPE_ID(MatMulRVVPass)
  StringRef getArgument() const final {
    return "matmul-rvv";
  }
  StringRef getDescription() const final {
    return "MatMul RVV vectorization optimization.";
  }
  MatMulRVVPass() = default;
  MatMulRVVPass(const MatMulRVVPass &) {}
  explicit MatMulRVVPass(int64_t affineVectorSizeParam, int64_t kBlockSizeParam) {
    affineVectorSize = affineVectorSizeParam;
    kBlockSize = kBlockSizeParam;
  }
...
void MatMulRVVPass::runOnOperation() {
...
void registerMatMulRVVPass() {
  PassRegistration<MatMulRVVPass>();
}
} // namespace buddy
} // namespace mlir