...
constexpr size_t ParamsSize = 1235814432;
constexpr size_t MaxVocabSize = 128256;
...
void printTokenIds(const std::vector<size_t> &ids, const std::string &name) {
  std::cout << "\033[33;1m[" << name << " Token IDs]\033[0m ";
...
int main() {
  ...
  /// Get user message.
const std::vector<size_t> fixedInputIds = {
    128000, 15339
};
  ...
  std::vector<size_t> generatedIds;
  ...
  inputContainer.assignTokenIds(fixedInputIds, MaxTokenLength);
  loadParameters(paramsDir, paramsContainer);

    printTokenIds(fixedInputIds, "Input");
...
std::cout << "Token ID: " << maxIndex << " | "
          << "Time: " << inferenceTime.count() / 1000 << "s" << std::endl;
    ...
    generatedIds.push_back(maxIndex);
    free(resultContainer.release());
  }

  /// Print the final result
  printTokenIds(fixedInputIds, "Input");
  printTokenIds(generatedIds, "Generated");
  return 0;
}
