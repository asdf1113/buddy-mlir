...
function(buddy_add_model)
  cmake_parse_arguments(
    MDL                                      # prefix
    ""                                       # flags
    "NAME;SPEC;RUNNER_SRC;RUNNER_PLUGIN_SRC;RUNNER_HDR;SERVING_PLUGIN_SRC;SERVING_LIBRARY;EMBEDDING_PLUGIN_SRC;EMBEDDING_LIBRARY;MASKED_LM_PLUGIN_SRC;MASKED_LM_LIBRARY;TRANSCRIPTION_PLUGIN_SRC;TRANSCRIPTION_LIBRARY;HF_CONFIG;LOCAL_MODEL;BUILD_DIR;MLIR_DIR;NUM_THREADS;LLC_ATTRS;COMPILE_JOBS;PARAMETER_PACK_DTYPE;TIERED_KV_CACHE;MODEL_KIND;IMPORT_SCRIPT;MANIFEST_SCRIPT;LOCAL_MODEL_ENV;MODEL_SO_NAME;TEMPLATE_PARTITION_CAPABLE"
    ...
    foreach(_rank RANGE 0 ${_MDL_LAST_TP_RANK})
      ...
      add_custom_command(
        ...
        DEPENDS
          ...
          "${MDL_MANIFEST_SCRIPT}"
        COMMENT
          "[${MDL_NAME}] Generating rank${_rank}.rhal.mlir (scheduled RHAL manifest)"
        VERBATIM
      )

      set(_RANK_STAGE4_DEPS
        rax-pack
        "${_rank_rhal}"
      ...
      if(SERVING_PLUGIN_TARGET)
        list(APPEND _RANK_STAGE4_DEPS ${SERVING_PLUGIN_TARGET})
      endif()

      list(APPEND _RANK_STAGE4_DEPS ${MDL_EXTRA_STAGE4_DEPS})

      add_custom_command(
        OUTPUT "${_rank_rax}"
        COMMAND "${CMAKE_BINARY_DIR}/bin/rax-pack"
                "${_rank_rhal}" -o "${_rank_rax}" ${RAX_PACK_ARGS}
        DEPENDS ${_RANK_STAGE4_DEPS}
